...
**Notes**:
`(void*)-1` is typecasting the number `-1` to a memory address that is of the type `void*` so that the compiler can recognize it. 

`(void*)-1` points to the location of the end of the possible memory range. 

`(void* )1` points to the very start, 1st location of the memory. it belongs to the kernel and is restricted. Using it may lead to segmentation faults and crashes. 

`(void*)0` points to NULL, aka nothing. 
